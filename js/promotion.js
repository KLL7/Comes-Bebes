document.addEventListener('DOMContentLoaded', () => {
    ...
    items.forEach(item => {
        ...
        originalPrices.push(Array.from(priceElements).map(priceElement => parseFloat(priceElement.textContent.replace('R$', '').replace(',', '.'))));
      });
    ...
    function revertPrices() {
        items.forEach((item, index) => {
            const priceElements = item.querySelectorAll('.item-price');
            priceElements.forEach((priceElement, priceIndex) => {
            priceElement.textContent = `R$ ${originalPrices[index][priceIndex].toFixed(2).replace('.', ',')}`;
          });
        });
    }
    ...
    function startPromotion() {
        const discounts = [0.20, 0.40, 0.70];
...
// Timer para indicar o início de um novo intervalo 
    function timePromotion() {
        ...
        const randomInterval = promotionIntervals[Math.floor(Math.random() * promotionIntervals.length)];
        setTimeout(() => {
            startPromotion();
            timePromotion(); 
      }, randomInterval);
